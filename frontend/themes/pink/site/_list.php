...
$category = isset($model->cats[0])?$model->cats[0]: Category::find()->where(['id'=>77])->one();
...
<div class="item-image">
    <?php if(empty($model->image)):?>
        <?php
            ...
            unset($images[0]);
        ?>
        <a href="<?=Url::to(['/product/view','category'=>$category->slug,'slug'=>$model->slug]);?>"><?= Html::img(UImage::getResizedImage($first,446,318),['alt'=>$alt])?></a>
    <?php else:?>
        <?php if($model->baget == 1):?>
        <a href="<?=Url::to(['/product/view','category'=>$category->slug,'slug'=>$model->slug]);?>">
...
<div class="item-actions clearfix">
    <a class="more" href="<?=Url::to(['/product/view','category'=>$category->slug,'slug'=>$model->slug]);?>">Подробнее</a>
